...
class ModelPipeline(Pipeline):
    ...
    async def process_agnostic_mask(self, parameter: dict) -> int:
        ...
        model_parse, _ = get_humanparsing_runtime().infer(file_path)
        model_metadata: ModelImageMetadata = self.get_file_upload_metadata(file_upload.id)
        pose_data = json.loads(model_metadata.keypoints)
        mask, mask_gray = get_mask_location('hd', self.category_dict_utils[0], model_parse, pose_data, radius=5)
        ...
        agnostic_file_v2_path = f"{image_id}_agnostic.{self.AGNOSTIC_FILE_EXTENSION}"
        masked_vton_img.save(agnostic_file_v2_path)
